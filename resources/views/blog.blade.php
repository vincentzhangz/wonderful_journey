...
@section('content')
    <div class="container mx-auto mt-5">
        ...
        <a class="inline-flex items-center h-8 px-4 m-2 text-sm text-indigo-100 transition-colors duration-150 bg-indigo-700 rounded-lg focus:shadow-outline hover:bg-indigo-800"
           href="{{route('newBlog')}}">Create new Blog</a>
        @if(count($article) !=0)
            ...
            <span
                class="lg:hidden absolute top-0 left-0 bg-blue-200 px-2 py-1 text-xs font-bold uppercase">Title</span>
                            <a href="{{route('full',['id'=>$a->id])}}" class="text-blue-500">{{$a->title}}</a>
                        </td>
                        <td class="w-full lg:w-auto p-3 text-gray-800 text-center border border-b text-center block lg:table-cell relative lg:static">
                            <a class="inline-flex items-center h-8 px-4 m-2 text-sm text-indigo-100 transition-colors duration-150 bg-indigo-700 rounded-lg focus:shadow-outline hover:bg-indigo-800"
                               href="{{route('deleteBlog',['id'=>$a->id])}}">Delete</a>
                        </td>
                    </tr>
                @endforeach
                </tbody>
